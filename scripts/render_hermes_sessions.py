...
def write_enzyme_config(output: Path) -> None:
    config = """# Generated by hermes-session-markdown.
vault:
  roots:
    - path: "."
...
"""
    (output / "enzyme.session-markdown.yaml").write_text(config, encoding="utf-8")


def write_index(output: Path) -> None:
    files = sorted(output.rglob("*.md"))
    entries = [p for p in files if p.name != "index.md"]
    lines = ["---", f'created: "[[{dt.date.today().isoformat()}]]"', f'rendered_at: "{utc_now()}"', 'kind: "session-markdown-index"', "---", "", "# Session Markdown Index", ""]
    current_dir = None
    for p in entries:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--no-enzyme-config", dest="enzyme_config", action="store_false", help="Do not write enzyme.session-markdown.yaml")
